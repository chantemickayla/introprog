...
function remove_selection(x) {
    console.log(x);
    if (quantities[x] <= 0) {
        alert("Items unavailable to be deducted")
    } else {
        quantities[x] = quantities[x] - 1;
        totals[x] = prices[x] * quantities[x];
        totalOrderAmt -= prices[x];
...
function displayTotal() {
    document.getElementById("total").innerHTML = "<br/><br/>Totalorder amount is R" + totalOrderAmt.toFixed(2);
}

function display_all() {
    var myTable = "<table><th style='width: 100px; color: red; textalign: right;'>Num</th>";
    ...
    myTable += "</table>";
    myTable += "<br/><br/><p>Total Order Amount:R " + totalOrderAmt + "</p>";
    //document.write(myTable);
    document.getElementById("demo").innerHTML = myTable;
    document.getElementById("checkTotal").onclick = displayTotal;
}
